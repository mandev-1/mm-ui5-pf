...
const DESTINATION_MAP: Record<string, { name: string; tagline: string; image: string }> = {
...
function mapAmadeusToDeal(d: { destination: string; departureDate?: string; returnDate?: string; price?: string }, index: number): Deal {
...
async function getAmadeusToken(apiKey: string, apiSecret: string): Promise<string> {
  const res = await axios.post(
    ...
    new URLSearchParams({ grant_type: 'client_credentials', client_id: apiKey, client_secret: apiSecret }),
...
async function fetchAmadeusDeals(origin: string, token: string, maxPrice?: number): Promise<Deal[]> {
  const params: Record<string, string> = { origin };
  if (maxPrice) params.maxPrice = String(maxPrice);
  const res = await axios.get('https://test.api.amadeus.com/v1/shopping/flight-destinations', {
    params,
    headers: { Authorization: 'Bearer ' + token }
  });
  const data = res.data?.data || [];
  return data.slice(0, 15).map((d: { destination: string; departureDate?: string; returnDate?: string; price?: string }, i: number) => mapAmadeusToDeal(d, i));
}

/** Fetch deals from the Docker flight-scraper service (when FLIGHT_SCRAPER_URL is set). */
async function fetchScraperDeals(baseUrl: string, origin: string): Promise<{ lombok: Deal[]; surfing: Deal[] }> {
 ...
 */
export async function getTopDeals(origin?: string): Promise<{ lombok: Deal[]; surfing: Deal[] }> {
